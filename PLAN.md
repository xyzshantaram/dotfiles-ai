...
- [ ] **W16 per-role routing + profile fallback chains — the `profiles` host plugin + `@visol-456/dsh-llm-fallback`.** **Coded 2026-08-21 late; build-, schema-, and seed-dry-run verified only. Live check pending the end-of-session restart.** `profiles` registers `coder`/`tester`/`researcher` tools on the HOST plane (web-profile patch row), so every agent preset gets them without forking a preset composition (user call: presets have no patch semantics, and a fork would bake one route into one preset). Each tool starts a leaf child (`maxDepth: 1`) over the same `spawn` seam as the shipped `subagent` tool, background-by-default via `startContinuable`, foreground on `run_in_background: false`. The route HEAD resolves AT CALL TIME: patch-row role pin → the shared `profile` settings namespace's active entry (see.ts owns that namespace; this plugin reads it with `ctx.settings.get`, never registering it twice) → inherit `agent-default-model`. FAILOVER EXECUTION is delegated to `@visol-456/dsh-llm-fallback` (user found it; source read before trusting: hooks the agent loop's request-error recovery, head never rewritten, circuit breaker, hot-rebuilding settings namespace `llm-fallback`, web UI page) — added to sync.sh's E4 install step; its peerDependencies pin rc.6, same watch-item as dsh-worktree. On a `profile.active` flip the profiles plugin pushes the new head into `agent-default-model` AND rewrites `llm-fallback`'s list with the active entry's tail, so one switch moves the main-agent default, the fallback order, and every child head together. sync.sh seeds both namespaces FIRST RUN ONLY: `profile` with chains (work = meridian opus → sonnet → zen x-preview-f-free; personal = zen deepseek-v4-flash-free → opencode-go deepseek-v4-flash → official deepseek PENDING a provider block in settings.yaml) and `llm-fallback` with the active personal tail. see.ts schema now accepts chain-shaped entries. Personas stay OUT of the plugin: the coder/tester/researcher skills remain the one source of truth. *Evaluate after restart:* the three tools appear in a fresh session; one background dispatch through `coder` returns a child id and settles on deepseek-v4-flash-free; a forced failure falls over to opencode-go flash; flipping `profile.active` to work moves new sessions to opus and realigns the fallback list.
...
- **Completed and verified this session (committed 2026-08-21 late):**
- **Profiles router BUILT (supersedes the earlier "build it first" decision).** User redirected the design mid-build: NO preset fork — the role tools live on the host plane like the rest of the bundle, and the work/personal choice stays runtime settings data reachable from ANY preset. See ticket W16 for the shape. Rerun list unchanged: W13 spike, W8 reject-with-comment, W14 caffeine fix, and E4 completion (add `@xgone/dsh-remote`, see below) — all now wait only on the single end-of-session restart that mounts the new host-plane plugins.
...
- **Commit state:** the four agreed groups landed as a61e899 (sync.sh aidos git-install + E4 staging), 7ee7102 (W7+W15+E5 plugin fixes), 92e54f1 (Phase C+D skills and wiring), 92016f8 (E4/E5 gating skills, conflict table, plan session state). The W16 profiles-router work (plugins/profiles.ts, plugins/profile-routes.ts, see.ts chain schema, build.mjs entry, sync.sh rows + seeds, this ticket) lands as one commit with this plan update. Nothing is pushed. Remaining before the reruns: one `dsh web` restart after a sync.sh run, and the official-DeepSeek provider block (baseURL, apiKeyEnv, model id) that the personal chain's third candidate waits on.
...
- **`@visol-456/dsh-llm-fallback` v0.1.2 cannot load on rc.8 (client half).** Its built client bundle requires `@deepseek-ai/dsh-client-web-react`, which rc.8 does not serve ("client-modules: require(...) missed the module table"), so the loader refuses the whole entry at boot. Host-side logic is unaffected; the Settings-page client bundle is the blocker. Install gated out of sync.sh until upstream rebuilds against rc.8. Until then there is NO automatic LLM failover anywhere: profiles still picks heads per role/profile, and its flip-sync writes to the `llm-fallback` namespace are absorbed no-ops.
