...
// The ElectronHub fold (ehSectionModel and its helpers) lives in
// ./eh-section-model.ts, dependency-free so the table test can import it.
// client.tsx only renders from the model; the state decision lives there.
...
function renderEhSection(ehUsage, ehModels) {
  ...
  return (
    <div className="ocgs-section">
      ...
      {errorLine ? <div className="dsp-err">{errorLine}</div> : null}
      {model.emptyLine ? <div className="ocgs-note">{model.emptyLine}</div> : null}
      {model.notes.map(function (note, ni) {
        return (
          <div className="ocgs-note" key={"eh-n-" + ni}>
            {note}
          </div>
        );
      })}
