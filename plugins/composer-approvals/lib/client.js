...
// plugins/composer-approvals/src/badge.ts
var ORDER = ["escalated", "rewrite", "approval", "question", "none"];
function rankOf(tone) {
  const at = ORDER.indexOf(tone);
  return at === -1 ? ORDER.length : at;
}
function approvalToneOf(reason) {
...
function badgeToneOf(approvalReasons, questionCount) {
  let best = "none";
...
/* BADGE TONE (#135), replacing the composer ring entirely.
   No rule here touches [data-composer-card] any more, so the cross-plugin
   box-shadow contract #132 had to build is retired with the ring rather than
...
.composer-approvals-indicator {
  transition:
    ...
    color var(--dsh-badge-fade, 1600ms) ease-out,
...
.composer-approvals-indicator[data-fading] {
  opacity: 0.35;
}
...
function makeIndicator() {
  return function Indicator(props) {
    ...
    var approvalReasons = [];
    for (var ai = 0; ai < approvalRows.length; ai++) {
      var live = selectorTools.pendingOf(approvalRows[ai].key);
      approvalReasons.push(live === void 0 || live === null ? void 0 : live.payload?.reason);
    }
