...
/* R14.1 (owner, 2026-09-18): panels stack flush, so the RUN reads as one
   ...
   corners of the stack are rounded, every seam between panels is square.
   A panel cannot express this with :first-child/:last-child on itself:
   client.tsx mounts EACH panel in its own wrapper div and appends the
   heredoc/argument bodies after it inside that wrapper, so every panel is
   already :first-child of its wrapper, and :last-child would turn on and
   off with whether the statement happens to carry a heredoc. The wrapper
   class carries the position instead: a wrapper PRECEDED by a wrapper is
   not first, and one FOLLOWED by a wrapper is not last. A lone panel
   matches neither rule and keeps all four corners. */
.prim-panel{box-sizing:border-box;border:none;
  border-radius:0;background:var(--dsw-alias-bg-base);
  margin-top:.125rem;margin-bottom:.125rem;padding:.25rem .375rem;}
.prim-panel-wrap > .prim-panel{border-radius:.75rem;}
.prim-panel-wrap ~ .prim-panel-wrap > .prim-panel{
...
/* R13.1 history, kept because the reasoning still governs this line. Four
   declarations were tried and dropped (padding-left, padding-right,
   display:inline-block on the pair, transform:translateX(-0.75rem)): a
   translateX moves PAINT ONLY, so every measured width, DP decision and edge
   coordinate would have kept describing the un-shifted box.
   R14.2 (owner, 2026-09-18): the 0.3rem margin-left this line carried is now
   GONE too \u2014 the icon and the label sit flush. The inline-block stays: it is
   what makes the pair one measurable box, and dropping it would reopen the
   R12.2 split the wrapper exists to prevent. Because the margin was real
   layout rather than paint, removing it needs no compensating constant \u2014 the
   measurer reads the same CSS the browser does, so the model and the browser
   keep agreeing (the round-8 rule) with the margin at zero. */
.hd-pair .node-text{display:inline-block;}
...
function errorSummaryOf(state, errorText) {
  return state === "error" && errorText !== null && errorText !== "" ? firstLineOfError(errorText) : void 0;
}
...
function ReadRow(props) {
  ...
  var errorSummary = errorSummaryOf(state, errorText);
...
function stripBashGraphRoot(cssText) {
  return String(cssText).replace(/(^|\})\s*:root[^{]*\{[^}]*\}/g, "$1").replace(/(^|\})\s*html\[data-theme="[^"]*"\]\s*\{[^}]*\}/g, "$1");
}
