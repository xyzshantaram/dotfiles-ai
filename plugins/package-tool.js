...
// plugins/package-tool.ts
var import_semver = __toESM(require_semver2());
import { join, relative, sep } from "node:path";
import { defineTool } from "@deepseek-ai/dsh-tools";
var name = "package-tool";
var inject = ["tools", "shell", "fs"];
var NODE_MARKERS = [
  ["pnpm-lock.yaml", "pnpm"],
  ["yarn.lock", "yarn"],
  ["bun.lock", "bun"]
...
var FALLBACK_MANAGER = {
  nodejs: "npm",
  rust: "cargo",
  python: "pip",
  go: "go"
};
function confineCwd(sessionCwd, requested) {
...
async function requireManifest(ctx, ecosystem, cwd, signal) {
  const candidates = MANIFEST_FILES[ecosystem];
  for (const name2 of candidates) {
    try {
      const target = await ctx.fs.resolve(name2, { cwd, ...signal ? { signal } : {} });
...
async function detectManager(ctx, ecosystem, cwd, signal) {
  ...
  for (const [marker, manager] of markers) {
    try {
      ...
      const info = await ctx.fs.stat(target, signal);
      if (info !== void 0) return manager;
    } catch {
    }
  }
  return FALLBACK_MANAGER[ecosystem];
}
async function runCommand(ctx, command, cwd, signal) {
...
async function resolveLatest(ctx, manager, packageName, cwd, signal) {
  switch (manager) {
    case "npm":
    case "pnpm":
    case "yarn":
...
async function getInstalledVersion(ctx, ecosystem, manager, packageName, cwd, signal) {
  try {
    if (ecosystem === "nodejs") {
...
function buildCommand(manager, action, packageName, dev) {
  switch (manager) {
    ...
    case "pnpm":
      return action === "remove" ? `pnpm remove ${packageName}` : `pnpm add ${dev ? "-D " : ""}${packageName}`;
    case "yarn":
      return action === "remove" ? `yarn remove ${packageName}` : `yarn add ${dev ? "-D " : ""}${packageName}`;
    case "bun":
      return action === "remove" ? `bun remove ${packageName}` : `bun add ${dev ? "-D " : ""}${packageName}`;
    case "cargo":
    ...
    case "pip":
      return action === "remove" ? `pip uninstall -y ${packageName}` : `pip install --upgrade ${packageName}`;
    case "go":
      return action === "remove" ? `go mod edit -droprequire=${packageName}` : `go get ${packageName}@latest`;
  }
}
function apply(ctx) {
  ctx.tools.register(
    defineTool({
      name: "package",
      description: "Add, remove, or update a dependency in the current project. Detect the package manager, resolve the latest registry version, and run the change. add refuses when the package is already installed (use update instead). update requires an existing install and refuses to downgrade it. Also register a project task (a scripts entry) through this sanctioned tool. Never pass an exact version: this tool always resolves the latest registry version, or whatever an existing lockfile/manifest constraint the manager itself applies. Use this tool for every dependency change.",
      parameters: {
        ecosystem: {
          type: "string",
          required: true,
        ...
        action: {
          type: "string",
          required: true,
          description: "Action: add (refuses if the package is already installed), remove, update (requires an existing install, refuses to downgrade it), or add_task.",
        ...
        cwd: {
          type: "string",
          description: "Project directory, relative to the session cwd. Omit to use the session cwd itself. Must not be an absolute path and must not escape the session cwd via '..'."
        },
        dev: {
          type: "boolean",
          description: "nodejs only: install into devDependencies instead of dependencies. Default false."
        },
        taskName: {
          type: "string",
