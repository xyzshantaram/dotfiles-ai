...
async function getInstalledVersion(
  ...
  try {
    ...
    if (ecosystem === 'python') {
      const out = await runCommand(ctx, `${manager === 'poetry' ? 'poetry run pip' : 'pip'} show ${packageName}`, cwd, signal)
...
export function apply(ctx: Context): void {
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const installedVersion = await getInstalledVersion(ctx, args.ecosystem, manager, args.packageName, cwd, signal)
      ...
      const version = action === 'remove' ? undefined : await resolveLatest(ctx, manager, args.packageName, cwd, signal)
      ...
      if (version !== undefined && installedVersion !== undefined && semverLt(version, installedVersion)) {
        const manualCommand = buildCommand(manager, 'add', `${args.packageName}@${installedVersion}`, dev)
        throw new Error(
