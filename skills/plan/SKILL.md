...
- **Prior-art check before a build ticket.** Before creating a ticket that builds a non-trivial capability, run the prior-art check defined in the `software-engineering` skill: three searches in order (platform, this repository, library), triggered by generality not size, findings must cite. Record the finding in the ticket — the citations are part of its context and stop a later review from relitigating the build decision. When the finding is an existing mechanism, the ticket also carries the adopt-vs-build recommendation that was put in front of the user. A build ticket with no prior-art finding for a general-purpose mechanism is not ready to dispatch.
