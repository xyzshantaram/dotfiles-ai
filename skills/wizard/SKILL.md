...
whenToUse: The user wants an interactive bash wizard that walks a human through manual steps only they can perform. Use it for provisioning infrastructure, setting up credentials or CI secrets, walking an unfamiliar third-party dashboard, or running a one-off migration or cutover. Do not invoke it for steps the agent can perform itself.
---
