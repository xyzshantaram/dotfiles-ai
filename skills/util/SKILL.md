...
This skill gates four zero-dependency host tools: `tool-time`, `tool-regex`,
`tool-markdown`, and `tool-encoding`. Each tool stays hidden from the model
...
|---|---|---|
| `tool-time` | `github:omdsh-dev/dsh-tool-time` | ISO 8601 parsing, timezone conversion, calendar arithmetic, duration difference |
| `tool-regex` | `github:omdsh-dev/dsh-tool-regex` | Test, extract, replace, static interpretation; runs under a worker with a hard timeout |
| `tool-markdown` | `github:omdsh-dev/dsh-tool-markdown` | HTML-to-Markdown and Markdown-to-HTML, GFM tables, table-of-contents generation, allow-list-based sanitization |
| `tool-encoding` | `github:omdsh-dev/dsh-tool-encoding` | base64, URL encoding, hex, hash functions, UUID generation |
...
- State which of the four tools you are about to call and why, before you
  ...
  string that could go through `tool-time` or a regex).
