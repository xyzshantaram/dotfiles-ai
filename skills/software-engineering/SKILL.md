...
2. **Medium or large feature requests**: before you write any code,
   - **Run the prior-art check before planning the build.** Before a ticket exists for a non-trivial capability, search for the one that already exists, in this order:
...
- **Dispatching mutation testing: the nonce is YOURS to mint.** When a ticket's evidence leans on test coverage, dispatch mutation testing rather than accepting "the tests pass" — a test that cannot fail buys false confidence. The runner is `scripts/mutation-test.ts`; the coverage floor and the exact invocation live in the `tester` skill. Link to them, never copy the numbers here, or the two drift and nobody knows which is authoritative. What belongs to YOU: generate a fresh random token per dispatch and put it in the brief. Never reuse one and never let the worker choose it — the token is the only thing distinguishing a real run from an invented or replayed report, and both have happened. Name the exact `--test` command. Require the report to echo the token verbatim AND show its full command line, because `--allow-survivors` makes exit 0 ambiguous, so an exit code alone proves nothing. Treat a missing or stale token as no report at all. (Until you read `tester`, none of this is visible to you: it is written from the worker's side, which is why it kept being skipped.)
...
- **Hand the subagent the verified APIs; it must not read library code.** When the brief needs a library, service, or harness API, the orchestrator verifies the shape once (read the `.d.ts`/source in the main session, or dispatch `researcher` for it) and pastes the exact facts into the brief: the type/interface shape, the field names, the method signatures, the return shapes, with file/line citations. The subagent then never greps `node_modules` or the installed dsh packages. In the brief, say explicitly: "API facts verified, do not re-research; if you need a fact not in this brief, report the gap instead of reading library code." Reading lots of library code is how a subagent's context bloats — the API knowledge belongs to the orchestrator, not the leaf worker.
  **You may only write that sentence over facts you actually checked, in this session, for THIS target.** Label anything else as assumed and invite the subagent to re-derive it. Asserting verification you did not perform converts your mistake into the worker's instruction, and the worker will obey: dispatching #85 the orchestrator wrote "Verified constraints — trust these, do not re-derive" over the claim that react/react-dom are external for `plugins/profiles-client`. It is true of `wrapClientBundle` and false for that plugin, which is built as a plain IIFE (`build.mjs:440-456`). Had the subagent complied, it would have bundled a React-importing library whose hoisted `__require("react")` throws before the plugin registers — a silently dead panel that `node build.mjs` and `tsc` both report as green. It surfaced only because the subagent disobeyed and re-derived. A fact is "verified" for the target you checked, not for the family it belongs to.
