...
3. **Delegate execution**, via the Task tool:
   ...
   - **Split test-writing and implementation into two separate `coder` dispatches when a ticket calls for new tests.** First dispatch a `coder` with only the test-writing brief (the contract/evaluation criteria settled during grilling, not the intended implementation approach) to produce the test module, naming the exact command that runs it. Then dispatch a second, fresh `coder` — deliberately *not* reusing the first dispatch's `task_id` — handed the finished test file and the implementation brief, to write code that satisfies it. The two stay independent on purpose: a single coder session writing both can quietly loosen a test to fit its own implementation, or shape tests around an approach it's already committed to, which defeats the point of having tests at all. `coder` never chooses on its own initiative what tests to run — the orchestrator always names the exact test/command in the brief.
   ...
   - **Trust subagent reports at each handoff; don't re-run what another step already checked.** If `coder` already ran build/typecheck/lint locally and reported the result, don't reflexively dispatch `tester` to redo the identical checks — dispatch it only for coverage `coder` didn't run (the fuller test suite, a broader scope, an environment `coder` doesn't have). The one place trust does not extend is the implementation-review boundary: before marking a ticket done, you still independently verify at least one concrete claim yourself, per the `plan` skill's review contract. That check is never satisfied by another subagent's report, however credible — everything upstream of it (coder-to-tester, coder-to-researcher) should assume good faith and skip duplicate work.
   ...
   - **Cut redundant context across dispatches.** A subagent invocation without a reused `task_id` starts from a blank context, so it re-reads files and re-derives conventions that an earlier subagent (or you, in the orchestrating session) already established — that's real token cost across the whole system, not just wall-clock. Reuse `task_id` to continue the same subagent session for related work touching the same area, instead of spinning up a fresh subagent per ticket. Front-load concrete file paths in the dispatch prompt when you already know them, rather than making the subagent grep/glob to rediscover them from scratch. Batch a ticket's related changes into one dispatch instead of splitting it across several small subagent calls. Skipping delegation entirely for small tasks (step 1) remains the single biggest lever. Exception: the test-writing → implementation handoff above is deliberately kept as two independent dispatches, never joined via `task_id` reuse — the isolation there is the safeguard, not overhead to cut.
...
**Agent:** *(runs `git add` / `git commit` for the two approved commits)* “Done — working tree is clean. I’m dispatching a `coder` with just the filtered-export test brief: the agreed columns, the filter-respecting requirement, and the command it should be checkable against.”

**Coder (test):** “Wrote the filtered-export test in `Invoices.test.tsx`; it currently fails since there's no export button yet.”
