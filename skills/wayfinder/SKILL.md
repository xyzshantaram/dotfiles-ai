---
name: wayfinder
description: Plan a huge chunk of work (more than one agent session can hold) as a shared map of decision tickets, and resolve them one at a time until the way to the destination is clear. Use when a loose idea arrives that is too big for one session and wrapped in fog.
whenToUse: A loose idea arrives that is too big for one session and the route to the destination is not yet visible. The user wants to chart decisions (not deliverables) and resolve them one at a time. Trigger phrases are "wayfinder", "chart this", "map the work", and "too big for one session".
---
...
In this harness there is no GitHub issue tracker by default. Use the **local-markdown tracker** instead of the issue-tracker operations above.

- **Map**: one markdown file in the durable scratch dir, e.g. `<scratch>/wayfinder/<name>.md`. The bundle's `plan` skill owns `PLAN.md` for execution tracking; keep wayfinder maps separate so charting never collides with execution.
